...
def cal_loss(pred, gold, smoothing):
    ...
    if smoothing:
        ...
        non_pad_mask = gold.ne(data.SPECIAL_TOKENS['<pad>'])
        loss = -(one_hot * log_prb).sum(dim=1)
        loss = loss.masked_select(non_pad_mask).sum()  # average later
    else:
        loss = F.cross_entropy(pred, gold, ignore_index=data.SPECIAL_TOKENS['<pad>'], size_average=False)
...
def main():
    train_loader = DataLoader(args.data, args.batch_size)
    ...
    for ep in range(start_ep, args.epoch):
        ...
        for batch in train_loader:
            ...
            board_feats_compress, board_feats = encoder(boards)
